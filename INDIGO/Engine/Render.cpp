...
namespace Engine
{
	...
	bool CRender::CreateObject()
	{
		if (!m_pFont)
		{
			HRESULT hFontRes = D3DXCreateFontA(m_pDevice, 12, 0, FW_NORMAL, 0, FALSE, RUSSIAN_CHARSET, OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FW_DONTCARE, "Verdana", &m_pFont);
		...
		if (!m_pGunFont)
		{
			HRESULT hFontRes = D3DXCreateFontA(m_pDevice, 16, 0, FW_NORMAL, 0, FALSE, RUSSIAN_CHARSET, OUT_DEFAULT_PRECIS, ANTIALIASED_QUALITY, DEFAULT_PITCH | FW_DONTCARE, "undefeated", &m_pGunFont);
	...
	void CRender::Circle2(int x, int y, float points, float radius, Color color)
	{
		float step = (float)M_PI * 2.0f / points;

		for (float a = 0; a < (M_PI * 2.0f); a += step)
		{
			Vector2D start(radius * cosf(a) + x, radius * sinf(a) + y);
			Vector2D end(radius * cosf(a + step) + x, radius * sinf(a + step) + y);
			Line(start, end, color);
		}
	}
	...
	void CRender::GunText(int x, int y, bool center, bool shadow, Color color, const char* format, ...)
	{
		if (m_pStateBlockText)
			m_pStateBlockText->Capture();
		...
		{
			rect.left++;
			m_pGunFont->DrawTextW(NULL, text, -1, &rect, DT_TOP | DT_LEFT | DT_NOCLIP, 0xFF000000);
			rect.top++;
			m_pGunFont->DrawTextW(NULL, text, -1, &rect, DT_TOP | DT_LEFT | DT_NOCLIP, 0xFF000000);
		};
		...
		if (center)
		{
			...
			m_pGunFont->DrawTextW(NULL, text, -1, &rec, DT_CALCRECT | DT_NOCLIP, dxTextColor);
			...
			m_pGunFont->DrawTextW(NULL, text, -1, &rec, DT_TOP | DT_LEFT | DT_NOCLIP, dxTextColor);
		}
		else
		{
			RECT rec =
