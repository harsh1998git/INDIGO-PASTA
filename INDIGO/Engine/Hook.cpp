...
namespace Engine
{
	...
	namespace Hook
	{
		...
		bool WINAPI Hook_CreateMove(float flInputSampleTime, CUserCmd* pCmd)
		{
			...
			//AA->PitchOverrideTick(pCmd);
			//AA->RealYawOverride(pCmd);

			g_Misc->Backtrack(pCmd);

			bool ret = Interfaces::ClientMode()->CreateMove(flInputSampleTime, pCmd);
			ClientModeTable.ReHook();

			if (Settings::Aimbot::weapon_settings[Client::iWeaponID].AimBot_pSilent)
				return false;
		...
		{			
			...
			if ( ctx && pCustomBoneToWorld && Client::g_pEsp && Settings::Esp::Visuals_Chams )
			{
