	...
	WEAPON_TYPE GetWeaponType( int iItemDefinitionIndex )
	{
		switch ( iItemDefinitionIndex )
		{
			...
			case WEAPON_KNIFE_PUSH:
				return WEAPON_TYPE_KNIFE;
			case WEAPON_GLOVES_BLOODHOUND:
				return WEAPON_TYPE_OTHER;
			case WEAPON_GLOVES_SPORT:
				return WEAPON_TYPE_OTHER;
			case WEAPON_GLOVES_DRIVER:
				return WEAPON_TYPE_OTHER;
			case WEAPON_GLOVES_WARPS:
				return WEAPON_TYPE_OTHER;
			case WEAPON_GLOVES_MOTO:
				return WEAPON_TYPE_OTHER;
			case WEAPON_GLOVES_SPEC:
				return WEAPON_TYPE_OTHER;
			case WEAPON_GLOVES_HYDRA:
				return WEAPON_TYPE_OTHER;
			default:
...
//[junk_enable /]
	...
	void ClanTag228(const char* tag)
	{
		SetMyClanTag(tag,"FOX_PROJECT");
	}
	...
	void ForceFullUpdate()
	{
		...
		ForceUpdate FullUpdate = (ForceUpdate)CSX::Memory::FindPatternV2("engine.dll", "A1 ? ? ? ? B9 ? ? ? ? 56 FF 50 14 8B 34 85");
		FullUpdate();
	}
	...
	void NameChange()
	{
		...
		*(int*)((DWORD)&Name->fnChangeCallback + 0xC) = 0;
		...
		if (Name)
		{
			if (steal)
			{
				Name->SetValue("-FOX"); // insert the Name of ur hack here or anything else u want to do
